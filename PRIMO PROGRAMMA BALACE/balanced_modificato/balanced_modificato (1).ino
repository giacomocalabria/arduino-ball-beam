...
#define TRIGGER_PIN  5                                                //Trigger Pin per sensore ultrasonico
#define ECHO_PIN 6                                                    //Echo Pin per sensore ultrasonico
#define MAX_DISTANCE 40                                               //Massima distanza da rilevare (cm)
const int servoPin = 9;                                               //Servo Pinint potPin = A0;                                                      //Analog Potentiometer Pin 


float Kp = 0.9;                                                       //Initial Proportional gain
...
void setup() {
  ...
  Setpoint = (15);                //Desired location of ball in terms of potentiometer redading
  myPID.SetMode(AUTOMATIC);                                          //Set PID object myPID to AUTOMATIC (see playground for more details)
  myPID.SetOutputLimits(70, 110);                                    //Set Output limits to 80 and 100 degrees. This keeps my servo from 
...
void loop()
{
  ...
  Setpoint = (15);                //Setpoint same as above in function setup() (see above for more details)
...
//**********************************************************************************************************************************************************
//readPosition() legge la posizione della palla e ritona nella posizion in cm. 
//**********************************************************************************************************************************************************
