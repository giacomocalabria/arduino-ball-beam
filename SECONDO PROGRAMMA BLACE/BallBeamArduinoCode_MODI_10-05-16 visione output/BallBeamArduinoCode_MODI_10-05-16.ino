...
float Ki = 0.3;                                                      //Initial Integral Gain
...
void setup() {
 Serial.begin(9600);                                                 //comunicazione seriale 
  ...
  myPID.SetOutputLimits(20,-30);                                     //Set Output limits to -80 and 80 degrees. 
}
void loop(){
  ...
  Serial.println(ServoOutput);//Writes value of Output to servo
} 
float readPosition() {
  delay(36); 
   //Don't set too low or echos will run into eachother.      
  unsigned int uS = sonar.ping_median(4);
  //ping_median() function is in class NewPing. It is a digital filter that takes the 
                                                                        //  the average of 4 (in this case) pings and returns that distance value. This helps 
      Serial.println(uS);                                                                  //  to filter out the pesy noise in those $5.00 UltraSonic Sensors. You know which
